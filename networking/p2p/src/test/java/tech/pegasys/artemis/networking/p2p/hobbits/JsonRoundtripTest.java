...
class JsonRoundtripTest {
  ...
  void roundtripBlockRoots() throws Exception {
    ObjectMapper mapper =
        new ObjectMapper(new BsonFactory())
            .registerModule(new RPCCodec.BytesModule())
    ...
    byte[] value = mapper.writerFor(BlockRoots.class).writeValueAsBytes(roots);
    BlockRoots read = mapper.readerFor(BlockRoots.class).readValue(value);
